...
class ReservacionSearch extends Reservacion
{
	...
	* Creates data provider instance with search query applied
		...
		$query = Reservacion::find()->andWhere('status=2')->andWhere('statusCheckOut=2')->andWhere(['fecha_entrada'=>date('Y-m-d')]);
	...
	* Creates data provider instance with search query applied
		...
		$query = Reservacion::find()->andWhere('status!=0')->andWhere('statusCheckOut!=0')->andWhere('status!=3')->andWhere('statusCheckOut!=3')->andWhere('status!=4')->andWhere('statusCheckOut!=4')->andWhere(['fecha_salida'=>date('Y-m-d')]);
